...
let pendingAttachments=[],cardAttachTarget=null;
const MAX_ATT=2*1024*1024;

function seed(){const H=36e5;tasks=[
  {id:id(),name:"Ship the onboarding redesign",priority:"high",deadline:now()+5*H,createdAt:now()-26*H,totalMs:64*60000,activeStart:null,attachments:[],links:[
  ...
  {id:id(),name:"Research grant draft",priority:"medium",deadline:now()+3*24*H,createdAt:now()-50*H,totalMs:0,activeStart:null,attachments:[],links:[
  ...
  {id:id(),name:"Read the saved long-reads",priority:"low",deadline:null,createdAt:now()-8*H,totalMs:22*60000,activeStart:null,attachments:[],links:[
...
function readFile(file){return new Promise(res=>{
  if(file.size>MAX_ATT){alert('"'+file.name+'" is too large (max 2 MB) — skipped.');res(null);return}
  const r=new FileReader();
  r.onload=e=>res({id:id(),name:file.name,type:file.type,size:file.size,dataUrl:e.target.result,addedAt:Date.now()});
  r.onerror=()=>res(null);r.readAsDataURL(file)})}

function attHtml(a,tid){
  const rm=`<button class="att-rm" data-rat="${a.id}" data-rt="${tid}" title="Remove">✕</button>`;
  if(isImg(a.type))return`<div class="att-item"><img src="${a.dataUrl}" class="att-thumb" data-lb="${a.id}" data-lt="${tid}" alt="${esc(a.name)}" title="${esc(a.name)}"><div class="att-meta"><span class="att-name">${esc(a.name)}</span><span class="att-sz">${fmtSz(a.size)}</span></div>${rm}</div>`;
  return`<div class="att-item att-file"><span class="att-ico">📎</span><div class="att-meta"><a class="att-dl" href="${a.dataUrl}" download="${esc(a.name)}">${esc(a.name)}</a><span class="att-sz">${fmtSz(a.size)}</span></div>${rm}</div>`}

function renderAttachPreview(){const el=document.getElementById("attachPreview");if(!el)return;
  el.innerHTML=pendingAttachments.map(a=>{
    const rm=`<button class="att-rm att-rm-pre" data-pid="${a.id}">✕</button>`;
    if(isImg(a.type))return`<div class="att-item"><img src="${a.dataUrl}" class="att-thumb" alt="${esc(a.name)}"><div class="att-meta"><span class="att-name">${esc(a.name)}</span><span class="att-sz">${fmtSz(a.size)}</span></div>${rm}</div>`;
    return`<div class="att-item att-file"><span class="att-ico">📎</span><div class="att-meta"><span class="att-name">${esc(a.name)}</span><span class="att-sz">${fmtSz(a.size)}</span></div>${rm}</div>`}).join("");
  el.querySelectorAll(".att-rm-pre").forEach(b=>b.addEventListener("click",()=>{pendingAttachments=pendingAttachments.filter(a=>a.id!==b.dataset.pid);renderAttachPreview()}))}
...
function addRow(){const i=linkRows.children.length+1;const row=document.createElement("div");row.className="linkrow";
  ...
  u.addEventListener("paste",e=>{const t=e.clipboardData.getData("text");
...
fileInput.addEventListener("change",async e=>{
  const reads=await Promise.all([...e.target.files].map(readFile));
  pendingAttachments.push(...reads.filter(Boolean));renderAttachPreview();fileInput.value=""});

const cardFileInput=document.getElementById("cardFileInput");
cardFileInput.addEventListener("change",async e=>{
  if(!cardAttachTarget)return;
  const t=tasks.find(x=>x.id===cardAttachTarget);if(!t)return;
  const reads=await Promise.all([...e.target.files].map(readFile));
  if(!t.attachments)t.attachments=[];
  t.attachments.push(...reads.filter(Boolean));render();save();
  cardFileInput.value="";cardAttachTarget=null});

const lightboxEl=document.getElementById("lightbox");
document.getElementById("lightbox-close").addEventListener("click",()=>lightboxEl.style.display="none");
...
btn.addEventListener("click",()=>{
  ...
  taskName.value="";linkRows.innerHTML="";addRow();document.getElementById("deadline").value="";
...
function renderCards(){const list=ordered();
  ...
  cardsEl.style.setProperty("--numcards",list.length);
  list.forEach((t,idx)=>{const p=PRIO[t.priority],r=rel(t.deadline);
    ...
    el.innerHTML=`
      <div class="file ruled" style="background-position:0 26px">
        ...
        ${sessionRow(t)}
        ${t.attachments&&t.attachments.length?`<div class="att-list card-atts">${t.attachments.map(a=>attHtml(a,t.id)).join("")}</div>`:""}
        <div class="acts">
          <button class="openall" data-open="${t.id}">⇲ Open all ${pad2(t.links.length)}</button>
          <button class="del" data-del="${t.id}">discard</button>
          <button class="att-add" data-att-t="${t.id}">+ attach</button>
  ...
  cardsEl.querySelectorAll(".att-rm").forEach(b=>b.addEventListener("click",()=>{
    const t=tasks.find(x=>x.id===b.dataset.rt);
    if(t){t.attachments=t.attachments.filter(a=>a.id!==b.dataset.rat);render();save()}}));
  cardsEl.querySelectorAll("[data-att-t]").forEach(b=>b.addEventListener("click",()=>{
    cardAttachTarget=b.dataset.attT;document.getElementById("cardFileInput").click()}));
  cardsEl.querySelectorAll("[data-lb]").forEach(img=>img.addEventListener("click",()=>{
    const t=tasks.find(x=>x.id===img.dataset.lt);if(!t)return;
    const a=t.attachments.find(x=>x.id===img.dataset.lb);if(!a)return;
    document.getElementById("lightbox-img").src=a.dataUrl;
    document.getElementById("lightbox").style.display="flex"}));

  // scroll-driven stack animation — JS version from .claude/skills/SKILL.md
  if("ViewTimeline" in window){
    const files=[...cardsEl.querySelectorAll(".docket .file")];
    const n=files.length;
    const vt=new ViewTimeline({subject:cardsEl,axis:"block"});
    files.forEach((file,index0)=>{
      const index=index0+1;
      const reverseIndex0=n-index;
      file.animate(
        {transform:[`scale(1)`,`scale(${1-0.1*reverseIndex0})`]},
        {timeline:vt,fill:"forwards",
         rangeStart:`exit-crossing ${(index0/n*100).toFixed(4)}%`,
         rangeEnd:`exit-crossing ${(index/n*100).toFixed(4)}%`}
      );
    });
  }
}
...
(()=>{
  ...
  const modeAsk=document.getElementById("ai-mode-ask");
  const modeAct=document.getElementById("ai-mode-act");
  let aiMode="ask";

  modeAsk.addEventListener("click",()=>{
    aiMode="ask";
    modeAsk.classList.add("active"); modeAct.classList.remove("active");
    logo.textContent="◈ CLAUDE"; logo.classList.remove("act-mode");
    input.placeholder="Ask Claude — what should I focus on? · summarise my workload · …";
  });
  modeAct.addEventListener("click",()=>{
    aiMode="act";
    modeAct.classList.add("active"); modeAsk.classList.remove("active");
    logo.textContent="⚡ MAC ACTION"; logo.classList.add("act-mode");
    input.placeholder="Run action — open Safari · screenshot · set volume 50 · say hello · …";
  });
  ...
  async function go(){
    ...
    input.value=""; panel.classList.add("open"); send.disabled=true;

    if(aiMode==="act"){
      resp.innerHTML=`<span class="ai-thinking">⚡ interpreting action…</span>`;
      try{
        const r=await fetch("/api/action",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({command:msg})});
        const d=await r.json();
        if(d.ok){
          resp.innerHTML=`<div class="act-result ok"><span class="act-status">✓ executed</span><span class="act-desc">${esc(d.description)}</span><code class="act-cmd">${esc(d.command)}</code>${d.output?`<pre class="act-out">${esc(d.output)}</pre>`:""}</div>`;
        }else{
          resp.innerHTML=`<div class="act-result fail"><span class="act-status">✗ blocked</span><span class="act-desc">${esc(d.description||d.error||"Unknown error")}</span>${d.command?`<code class="act-cmd">${esc(d.command)}</code>`:""}</div>`;
        }
      }catch(e){resp.textContent="⚠ "+e.message}
    }else{
      resp.innerHTML=`<span class="ai-thinking">thinking…</span>`;
      try{
        const r=await fetch("/api/claude",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:msg,tasks})});
        if(!r.ok){const e=await r.json().catch(()=>({}));resp.textContent="⚠ "+(e.error||"Error");send.disabled=false;return}
        resp.textContent="";
